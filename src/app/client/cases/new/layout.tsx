...
export default function NewCaseLayout({
  children,
}: Readonly<{
    ...
    <div className="min-h-screen flex flex-col">
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-[#8EBA43]/30 dark:border-[#4d9731]/40 bg-gradient-to-r from-[#4d9731] to-[#8EBA43] dark:from-[#4d9731] dark:to-[#2A3132] backdrop-blur shadow-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <Link
            href={!loading && isLoggedIn ? "/client" : "/"}
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <Link
                  href="/login"
                  className="px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white font-semibold text-sm transition-all duration-300"
                >
                  ログイン
                </Link>
                <Link
                  href="/register"
                  className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#FD9731] to-[#f57c00] hover:from-[#f57c00] hover:to-[#FD9731] text-white font-semibold text-sm shadow-md hover:shadow-lg transition-all duration-300"
                >
                  会員登録
                </Link>
              </div>
      ...
      <div className="flex-1 flex bg-gradient-to-br from-[#8EBA43]/5 via-white to-[#4d9731]/5 dark:from-[#2A3132] dark:via-[#2A3132]/95 dark:to-[#4d9731]/10 pt-[73px]">
        ...
        {!loading && isLoggedIn && (
          ...
          {/* メインコンテンツ */}
        <main className={!loading && isLoggedIn ? "flex-1 ml-56" : "flex-1"}>
          <div className="mx-auto w-full max-w-7xl py-8">{children}</div>
        </main>
      </div>
    </div>
  );
